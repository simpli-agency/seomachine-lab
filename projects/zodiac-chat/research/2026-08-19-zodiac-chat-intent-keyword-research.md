...
**Источники данных:** DataForSEO (Labs keyword overview, Google Ads search volume, keyword suggestions, ranked keywords, live SERP), прямой запрос к сайту (HTTP)
**GSC:** недоступен — см. раздел «Блокеры»
...
1. Поднять GSC-доступ (см. блокеры) — без него не видно ни одного реального запроса и невозможны quick wins.
2. Создать `sitemap.xml` (сейчас 404) и добавить директиву `Sitemap:` в `robots.txt` (сейчас там только комментарии).
3. Решить, где живёт контент: `/blog/` или корневые лендинги. astroo.ai использует и то, и другое (`/synastry` + `/blog/...`).
...
**GSC — доступа нет.**
Проверены обе формы property: `sc-domain:zodiac-chat.com` и `https://zodiac-chat.com/`. Обе падают на одной и той же причине: **файл сервис-аккаунта, на который указывает `GSC_CREDENTIALS_PATH` в `.env`, на этой машине отсутствует** (каталог `credentials/` не создан — он gitignored и провижинится вне репозитория). Это инфраструктурный блокер, а не вопрос прав на property.

Что из-за этого **не сделано** (пункты 4 ТЗ): traffic footprint (clicks / impressions / CTR / avg position), 90d и 28/30d page-level строки, query+page строки, распределение позиций 4–10 / 10–30 / 30+, сверка sitemap vs GSC.

Чтобы разблокировать, нужно и то, и другое:
1. положить JSON сервис-аккаунта по пути из `GSC_CREDENTIALS_PATH`;
2. выдать e-mail этого сервис-аккаунта доступ (минимум Restricted) к property zodiac-chat.com в Search Console.

Частичная компенсация: текущая видимость оценена через DataForSEO Labs `ranked_keywords` (раздел 3). Но это оценка сторонней базы, а не фактические данные Google.

**Прочие ограничения:**
- `sitemap.xml` отдаёт 404 — сверять с GSC (когда он появится) будет нечего, пока карту не создадут.
- Эндпоинт DataForSEO Labs `domain_metrics` возвращает 404 на текущем тарифе — organic keywords count / ETV / domain rank по доменам недоступны. Данные раздела 3 получены через `ranked_keywords` (поле `total_count`).
